...
//	public int boardNumber = 0;
	 ...
	 * an argument. It reads data from the input file and provides lines that, when
	 * uncommented, will display exactly what has been read in from the input file.
	 * You can find these lines by looking for
	 * 
	 * @param inputFile
	 *            the path of the input file for the game
	 */
	...
	public void setParentBoard(GameBoard parentBoard) {
	...
	public GameBoard copy() {
		...
		copyBoard.setPlayBoard(copy2DArray(this.getPlayBoard()));
		copyBoard.setPieceCount(this.getPieceCount());
		copyBoard.setActionStates(new ArrayList());
		
		return copyBoard;
	}
	
	private int[][] copy2DArray(int [][] twoDArray) {
		
		int [][] copyArray = new int[twoDArray.length][twoDArray[0].length];
	 ...
	 * horizontally, vertically, and each direction diagonally. currently, it uses
	 * for loops, but i'm sure that it can be made more efficient.
	 * 
	 * @param player
	 *            the player whose score is being requested. valid values are 1 or 2
	 * @return the integer of the players score
	 */
	public int getScore(int player) {
		// reset the scores
		int playerScore = 0;
		int [] sz = this.getBoardSize();

		// check horizontally
		for (int i = 0; i < 6; i++) {
			
			for (int j = 0; j < 4; j++) { //From left.
	...
	public int getScore(int player, int numConsecutive) {
		
		// reset the scores
		int playerScore = 0, count = 0;
		int [] sz = this.getBoardSize();
		
		// check horizontally
		for (int i = 0; i < sz[0]; i++) {
			for (int j = 0; j < (sz[1] - numConsecutive + 1); j++) {
				
				count = 0;
				for (int k = 0; k < numConsecutive; k++) {
					if (this.playBoard[i][j + k] == player)
						count++;
				}
		...
		// check vertically
		for (int i = 0; i < (sz[0] - numConsecutive + 1); i++) {
			for (int j = 0; j < sz[1]; j++) {
				
				count = 0;
				for (int k = 0; k < numConsecutive; k++) {
					if (this.playBoard[i + k][j] == player)
						count++;
				}
		...
		for (int i = 0; i < (sz[0] - numConsecutive + 1); i++) {
			for (int j = 0; j < (sz[1] - numConsecutive + 1); j++) {
				...
				count = 0;
				for (int k = 0; k < numConsecutive; k++) {
					if (this.playBoard[i + k][j + k] == player)
						count++;
				}
		...
		for (int i = 0; i < (sz[0] - numConsecutive + 1); i++) {
			for (int j = 0; j < (sz[1] - numConsecutive + 1); j++) {
				...
				count = 0;
				for (int k = 0; k < numConsecutive; k++) {
					if (this.playBoard[i + numConsecutive - (k + 1)][j + k] == player)
						count++;
				}
	...
	/**
	 * Returns an integer array of size 2 containing the first element as row size
	 * and 2nd element as column size.
	 * 
	 * @return an integer array of size 2 containing the first element as row size
	 *         and 2nd element as column size.
	 */
	public int[] getBoardSize() {
		return new int[] { playBoard.length, playBoard[0].length };
	}

	/**
	 * Returns the size of rows if rowCol parameter is 1, else returns the size of
	 * columns if rowCol parameter is 2.
	 * 
	 * @param rowCol
	 *            1 = row size, 2 = column size.
	 * @return size of the rows or columns as per the parameter.
	 */
	public int getBoardSize(int rowCol) {
		return getBoardSize()[rowCol - 1];
	}
